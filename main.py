...
class demo(Frame):
    ...
    def create_widgets(self):
        ...
        self.frame_canvas1.grid(row=0, column=0, columnspan=2)
        #list of foods
        self.frame_list1 = Frame(self)
        self.frame_list1.grid(row=1, column=0, columnspan=2)
        ...
        #button(select)
        self.select_btn1 = Button(self,width=20)
        self.select_btn1['text'] = 'select'
        self.select_btn1['command'] = self.select_image1
        self.select_btn1.grid(row=2, column=0)
        #button(rotate)
        self.rotate_btn1 = Button(self,width=20)
        self.rotate_btn1['text'] = 'rotate'
        self.rotate_btn1['command'] = self.rotate_image1
        self.rotate_btn1.grid(row=2, column=1)
        #label
        self.selected_food1 = Label(self)
        self.selected_food1['text'] = 'Image is not selected'
        self.selected_food1.grid(row=3, column=0, columnspan=2)
        ##Right-----------------------------------------------------------------------------
        #food image
        self.frame_canvas2 = Canvas(self, bg='azure3', height=300, width=300)
        self.frame_canvas2.grid(row=0, column=2, columnspan=2)
        #list of foods
        self.frame_list2 = Frame(self)
        self.frame_list2.grid(row=1, column=2, columnspan=2)
        self.foods = glob.glob("data/**/*")
        ...
        #button(select)
        self.select_btn2 = Button(self,width=20)
        self.select_btn2['text'] = 'select'
        self.select_btn2['command'] = self.select_image2
        self.select_btn2.grid(row=2, column=2)
        #button(rotate)
        self.rotate_btn2 = Button(self,width=20)
        self.rotate_btn2['text'] = 'rotate'
        self.rotate_btn2['command'] = self.rotate_image2
        self.rotate_btn2.grid(row=2, column=3)
        #label
        self.selected_food2 = Label(self)
        self.selected_food2['text'] = 'Image is not selected'
        self.selected_food2.grid(row=3, column=2, columnspan=2)
        ##Calculator------------------------------------------------------------------------
        #Button
        self.calc_btn = Button(self,width=20)
        self.calc_btn['text'] = 'calculate similarity'
        self.calc_btn['command'] = self.calc_similarity
        self.calc_btn.grid(row=2, column=4)
        #Result
        self.result = Label(self, font=("Helvetica","13"))
        self.result['text'] = ''
        self.result.grid(row=3, column=4)
    ##Process-------------------------------------------------------------------------------
    ...
    def rotate_image1(self):
        self.image1 = self.image1.rotate(90)
        self.image1tk = ImageTk.PhotoImage(self.image1)
        self.frame_canvas1.create_image(
            0,
            0,
    ...
    def rotate_image2(self):
        self.image2 = self.image2.rotate(90)
        self.image2tk = ImageTk.PhotoImage(self.image2)
        self.frame_canvas2.create_image(
            0,
            0,
